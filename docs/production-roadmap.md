...
- [ ] 新建 `src/router/routes.tsx`，集中维护路径、标题、图标、页面组件和菜单属性。
- [ ] 从同一份元数据生成 React Router 路由和 ProLayout 菜单。
- [ ] 支持嵌套路由、默认跳转、懒加载、菜单隐藏和独立布局页面。
- [ ] 由匹配路由更新文档标题和面包屑，移除页面内重复标题逻辑。
- [ ] 将登录、403、404、500 等特殊路由明确标记为不显示在菜单中。
- [ ] 删除原有独立菜单配置，确保新增页面只修改一处。
- [ ] 保持 `BasicLayout` 只负责 ProLayout 插槽和布局状态，不承担权限计算。
- [ ] 清理未使用的路由、布局辅助代码。
